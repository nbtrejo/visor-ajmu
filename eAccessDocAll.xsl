  ...
  <body>
   <h2>USABILITY METRICS FOR DESKTOP APPLICATION: <xsl:value-of select="/usabilityRecord/@appName"/> version <xsl:value-of select="/usabilityRecord/@version"/> </h2>
    ...
    <table border="1">
      <tr bgcolor="#9acd32">
      	<th>Session Id</th>
        <th>TOTAL Access to Help per Task Execution per Session</th>        
      </tr>
	        	...
	        	<xsl:for-each select="./task[starts-with(@id, /usabilityRecord/tasksAnalized/taskName[1]/@name)]/log">
				 		...
				 		<xsl:for-each select="./logParcial">
				 			<xsl:if test="position() = last()">  
				 				<b>----Access to Help in Parcial Log:   </b>   	<xsl:value-of select="./@totalAccDoc"/> <br />
				 			</xsl:if>	
				 		</xsl:for-each>
				 		<xsl:for-each select="./logFinal">
				 			<xsl:if test="position() = last()">  
				 				<b>----Access to Help in Final Log:   </b>   	<xsl:value-of select="./@totalAccDoc"/> <br />
				 			</xsl:if>	
				 		</xsl:for-each>				 		
				</xsl:for-each>
