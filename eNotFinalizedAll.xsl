  ...
  <body>
   <h2>USABILITY METRICS FOR DESKTOP APPLICATION: <xsl:value-of select="/usabilityRecord/@appName"/> version <xsl:value-of select="/usabilityRecord/@version"/> </h2>
    ...
    <table border="1">
      <tr bgcolor="#9acd32">
      	<th>Session Id</th>
        <th>Percentage Tasks Not Finalized per Session and Task Name</th>        
      </tr>
      <xsl:for-each select="usabilityRecord/session">
        <tr>
        	...
        	<td> 	        	
				...
				<b>COUNT Task NOT Finalized: </b> <xsl:variable name="count_task_CrearNotFinalized" select="count(./task[starts-with(@id, /usabilityRecord/tasksAnalized/taskName[1]/@name) and @state='No Finalizada'])"> 
								  																		
								  </xsl:variable> <xsl:copy-of select="$count_task_CrearNotFinalized" /><br />
				...
				<b>Task NOT Finalized percent: </b> (COUNT/TOTAL)*100 = <xsl:copy-of select="($count_task_CrearNotFinalized div $count_task_Crear) * 100" /> %<br />
				
				<xsl:value-of select="$newline"/>
				 
		        <b>Task Name: </b>	<xsl:value-of select="/usabilityRecord/tasksAnalized/taskName[2]/@name"/> <br />
				 		 
				<b>COUNT Task NOT Finalized: </b> <xsl:variable name="count_task_EditarNotFinalized" select="count(./task[starts-with(@id, /usabilityRecord/tasksAnalized/taskName[2]/@name) and @state='No Finalizada'])"> 
								  
								  </xsl:variable> <xsl:copy-of select="$count_task_EditarNotFinalized" /><br />
				...
				<b>Task NOT Finalized percent: </b> (COUNT/TOTAL)*100  = <xsl:copy-of select="($count_task_EditarNotFinalized div $count_task_Editar)* 100" /> % <br />
        	</td>
        </tr>
      </xsl:for-each>
    </table>
