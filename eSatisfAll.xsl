  ...
  <body>
   <h2>USABILITY METRICS FOR DESKTOP APPLICATION: <xsl:value-of select="/usabilityRecord/@appName"/> version <xsl:value-of select="/usabilityRecord/@version"/> </h2>
    ...
    <table border="1">
      <tr bgcolor="#9acd32">
      	<th>Session Id</th>
        <th>Rate Tasks qualified as "Satisfied" per Session and Task Name</th>        
      </tr>
      <xsl:for-each select="usabilityRecord/session">
        <tr>
        	...
        	<td> 	        	
				...
				<b>COUNT Task Finalized Satisfied: </b> <xsl:variable name="count_task_CrearFinalizedS" select="count(./task[starts-with(@id, /usabilityRecord/tasksAnalized/taskName[1]/@name) and @state='Finalizada']/log/logFinal[@SatisDegree='Buena'])"> 
								  																		
								  </xsl:variable> <xsl:copy-of select="$count_task_CrearFinalizedS" /><br />
				...
				<b>Percent Tasks Finalized Satisfied: </b> (COUNT/TOTAL)*100 = <xsl:copy-of select="($count_task_CrearFinalizedS div $count_task_Crear) * 100" /> %<br />
				...
				<b>COUNT Task Finalized Satisfied: </b> <xsl:variable name="count_task_EditarFinalizedS" select="count(./task[starts-with(@id, /usabilityRecord/tasksAnalized/taskName[2]/@name) and @state='Finalizada']/log/logFinal[@SatisDegree='Buena'])"> 
								  
								  </xsl:variable> <xsl:copy-of select="$count_task_EditarFinalizedS" /><br />
				...
				<b>Percent Tasks Finalized Satisfied: </b> (COUNT/TOTAL)*100  = <xsl:copy-of select="($count_task_EditarFinalizedS div $count_task_Editar)* 100" /> % <br />
        	</td>
        </tr>
      </xsl:for-each>
    </table>
