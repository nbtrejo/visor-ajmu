  ...
  <body>
   <h2>USABILITY METRICS FOR DESKTOP APPLICATION: <xsl:value-of select="/usabilityRecord/@appName"/> version <xsl:value-of select="/usabilityRecord/@version"/> </h2>
    ...
    <table border="1">
      <tr bgcolor="#9acd32">
      	<th>Session Id</th>
        <th>Rate Tasks qualified as "Dissatisfied" per Session and Task Name</th>        
      </tr>
      <xsl:for-each select="usabilityRecord/session">
        <tr>
        	...
        	<td> 	        	
				...
				<b>COUNT Task Finalized Dissatisfied: </b> <xsl:variable name="count_task_CrearFinalizedD" select="count(./task[starts-with(@id, /usabilityRecord/tasksAnalized/taskName[1]/@name) and @state='Finalizada']/log/logFinal[@SatisDegree='Dissatisfied'])"> 
								  																		
								  </xsl:variable> <xsl:copy-of select="$count_task_CrearFinalizedD" /><br />
				...
				<b>Percent Tasks Finalized Dissatisfied: </b> (COUNT/TOTAL)*100 = <xsl:copy-of select="($count_task_CrearFinalizedD div $count_task_Crear) * 100" /> %<br />
				...
				<b>COUNT Task Finalized Dissatisfied: </b> <xsl:variable name="count_task_EditarFinalizedD" select="count(./task[starts-with(@id, /usabilityRecord/tasksAnalized/taskName[2]/@name) and @state='Finalizada']/log/logFinal[@SatisDegree='Dissatisfied'])"> 
								  
								  </xsl:variable> <xsl:copy-of select="$count_task_EditarFinalizedD" /><br />
				...
				<b>Percent Tasks Finalized Dissatisfied: </b> (COUNT/TOTAL)*100  = <xsl:copy-of select="($count_task_EditarFinalizedD div $count_task_Editar)* 100" /> % <br />
        	</td>
        </tr>
      </xsl:for-each>
    </table>
